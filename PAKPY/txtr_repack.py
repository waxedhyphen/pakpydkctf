...
import subprocess
import tempfile
from pak_core import PakError, parse_asset_chunks, build_chunk_raw, w64, be32
...
def _patch_head_mips_and_texture_size(payload, mip_sizes, texture_size):
    buf = bytearray(payload)
    if len(buf) < 36 or len(buf) % 4 != 0:
        raise PakError('HEAD-Chunk ist ungültig')
    mip_count = be32(buf, 28)
    if mip_count != len(mip_sizes):
        raise PakError(f'HEAD-Mip-Anzahl passt nicht | erwartet {mip_count} | gefunden {len(mip_sizes)}')
    pos = 32
    for size in mip_sizes:
        if pos + 4 > len(buf):
            raise PakError('HEAD-Chunk ist abgeschnitten')
        buf[pos:pos + 4] = int(size).to_bytes(4, 'big')
        pos += 4
    if pos + 4 > len(buf):
        raise PakError('HEAD-Chunk ist abgeschnitten')
    buf[pos:pos + 4] = int(texture_size).to_bytes(4, 'big')
    return bytes(buf)
...
def png_to_txtr_asset(original_asset, png_path):
    ...
    mip_sizes = [len(part) for part in mip_payloads]
    ...
    for chunk in parse_asset_chunks(original_asset):
        ...
        if chunk['tag'] == 'HEAD':
            payload = _patch_head_mips_and_texture_size(payload, mip_sizes, len(raw_gpu))
        elif chunk['tag'] == 'GPU ':
