...
def parse_actor_ref_blob(data):
    ...
    if len(tail) >= 50:
        try:
            values = struct.unpack('>12f', tail[2:50])
            transform = {'position': values[0:3], 'rotation_raw': values[3:9], 'scale': values[9:12], 'tail_hex': tail.hex()}
        except Exception:
            transform = None
...
def parse_comp(asset, off, limit):
    ...
    if end > limit or payload + 24 > end:
        return None
    uuid_hex = asset[payload:payload+16].hex()
    type_hash = asset[payload+16:payload+20].hex()
...
def parse_room_asset(asset):
    ...
    for section in iter_rfrm_children(asset, 32, root_end):
        short = {'index': section['index'], 'type': section['type'], 'off': section['off'], 'size': section['size'], 'payload_size': section['payload_size']}
    ...
    for component in components:
        ...
        for candidate in components:
            actor_refs = candidate.get('actor_refs')
            if not actor_refs:
                continue
            for ref in actor_refs['refs']:
                if ref['uuid_hex'] == component['uuid_hex']:
                    parents.append({'uuid_hex': candidate['uuid_hex'], 'name': candidate['name'], 'type_hash': candidate['type_hash'], 'layer_name': candidate['layer_name']})
...
def summarize_room(parsed, entry):
    info = parse_room_asset(get_entry_asset(parsed, entry))
    known = dcln_entry_map(parsed)
    resolved = sum(1 for ref in info['dcln_refs'] if ref['dcln_uuid_hex'] in known)
    actor_like = sum(1 for component in info['components'] if component.get('actor_refs'))
    return {'info': info, 'known_dcln_count': len(known), 'resolved_dcln_refs': resolved, 'actor_like_components': actor_like}

def format_room_info_lines(parsed, entry):
    summary = summarize_room(parsed, entry)
    info = summary['info']
    lines = []
    lines.append('ROOM-Analyse:')
    lines.append(f'- Version: {info["root_version_a"]} / {info["root_version_b"]}')
    lines.append(f'- Sections: {len(info["sections"])}')
    lines.append(f'- Layer: {len(info["layers"])}')
    lines.append(f'- Komponenten: {len(info["components"])}')
    lines.append(f'- Actor-ähnliche Komponenten: {summary["actor_like_components"]}')
    lines.append(f'- DCLN-Referenzen: {len(info["dcln_refs"])}')
    lines.append(f'- DCLN-Referenzen mit Eintrag im aktuellen PAK: {summary["resolved_dcln_refs"]}')
    if info['layers']:
        lines.append('')
    ...
    if info['dcln_refs']:
        ...
        for ref in info['dcln_refs'][:80]:
            status = 'im PAK' if ref['dcln_uuid_hex'] in dcln_entry_map(parsed) else 'nicht im PAK'
            lines.append(f'- {format_uuid_hex(ref["dcln_uuid_hex"])} | {status} | Layer {ref["layer_name"]} | {ref["component_name"]}')
        if len(info['dcln_refs']) > 80:
            lines.append(f'... {len(info["dcln_refs"]) - 80} weitere')
    return lines

def serializable_component(component):
    item = {k: v for k, v in component.items() if k not in ('raw_body',)}
    for prop in item.get('properties', []):
        prop.pop('value', None)
    return item

def write_components_tsv(path, components):
    lines = ['index\toffset\tlayer\ttype_hash\tuuid\tname\tproperty_count\tactor_ref_count\tparent_count']
    for index, comp in enumerate(components):
        ref_count = 0 if not comp.get('actor_refs') else len(comp['actor_refs']['refs'])
        lines.append(f'{index}\t0x{comp["off"]:X}\t{comp["layer_name"]}\t{comp["type_hash"]}\t{format_uuid_hex(comp["uuid_hex"])}\t{comp["name"]}\t{len(comp["properties"])}\t{ref_count}\t{len(comp.get("parents", []))}')
    Path(path).write_text('\n'.join(lines), encoding='utf-8', newline='\n')

def write_dcln_refs_tsv(path, refs, known):
    lines = ['index\tdcln_uuid\tstatus\tlayer\tcomponent\tcomponent_uuid\tcomponent_type_hash\tcomponent_offset\tparent_names']
    for index, ref in enumerate(refs):
        status = 'in_pak' if ref['dcln_uuid_hex'] in known else 'missing'
        parents = '; '.join(parent['name'] for parent in ref.get('parents', []))
        lines.append(f'{index}\t{format_uuid_hex(ref["dcln_uuid_hex"])}\t{status}\t{ref["layer_name"]}\t{ref["component_name"]}\t{format_uuid_hex(ref["component_uuid_hex"])}\t{ref["component_type_hash"]}\t0x{ref["component_off"]:X}\t{parents}')
    Path(path).write_text('\n'.join(lines), encoding='utf-8', newline='\n')

def write_room_report(path, entry, info, known):
    lines = []
    lines.append(f'ROOM: {entry.get("display_name") or entry.get("name") or entry["uuid_hex"]}')
    lines.append(f'UUID: {format_uuid_hex(entry["uuid_hex"])}')
    lines.append(f'Sections: {len(info["sections"])}')
    lines.append(f'Layer: {len(info["layers"])}')
    lines.append(f'Komponenten: {len(info["components"])}')
    lines.append(f'DCLN-Referenzen: {len(info["dcln_refs"])}')
    lines.append(f'DCLN im aktuellen PAK auflösbar: {sum(1 for ref in info["dcln_refs"] if ref["dcln_uuid_hex"] in known)}')
    lines.append('')
    lines.append('DCLN-Referenzen:')
    for ref in info['dcln_refs']:
        status = 'im PAK' if ref['dcln_uuid_hex'] in known else 'nicht im PAK'
        lines.append(f'- {format_uuid_hex(ref["dcln_uuid_hex"])} | {status} | Layer {ref["layer_name"]} | {ref["component_name"]} | COMP {format_uuid_hex(ref["component_uuid_hex"])}')
    Path(path).write_text('\n'.join(lines), encoding='utf-8', newline='\n')
...
def write_collision_mtl(path):
    ...
    for index, color in enumerate(palette):
        lines.append(f'newmtl room_collision_{index:03d}')
...
def write_collision_debug_obj(parsed, refs, known, obj_path, mtl_name):
    ...
    for ref_index, ref in enumerate(refs):
        ...
        col = exported % cols
        row = exported // cols
        offset = (col * spacing, 0.0, row * spacing)
        ...
        for vertex in dcln['vertices']:
            x, y, z = vertex['pos']
            lines.append(f'v {fmt_num(x + offset[0])} {fmt_num(y + offset[1])} {fmt_num(z + offset[2])}')
        last_mat = None
...
def describe_room_hit(asset, asset_off, uuid_hex):
    ...
    for component in info['components']:
        ...
        for prop in component['properties']:
            marker = bytes.fromhex(prop['value_hex']) if prop['size'] == 16 else None
            if marker is not None and prop['value_hex'] == uuid_hex:
                hit_prop = prop
                break
        return {'layer_name': component['layer_name'], 'component_name': component['name'], 'component_uuid_hex': component['uuid_hex'], 'component_type_hash': component['type_hash'], 'component_rel': rel, 'property_key': '' if hit_prop is None else hit_prop['key']}
    for layer in info['layers']:
        if layer['off'] <= asset_off < layer['off'] + layer['size']:
            return {'layer_name': layer['name'], 'component_name': '', 'component_uuid_hex': '', 'component_type_hash': '', 'component_rel': None, 'property_key': ''}
    return None
